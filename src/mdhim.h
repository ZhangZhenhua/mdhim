...
#define MAX_RANGE_SRVS (26*26) /* for string keys, using first two letters to
				* determine the range server, see mdhim_hash.c
				* */
...
#define SERVERS 5
#define SRVTAG 1
#define MAX_ISAM_FD_NUM 100
//#define DATABUFFERSIZE 2048
//#define KEYSIZE 255
#define DATABUFFERSIZE (64*1024)
#define KEYSIZE 1024
#define INDEXTABLEVALUESIZE 4096
#define UNLINKMAGICBIT (1UL << 63)
#define RESTOREMAGICBIT 0x7FFFFFFFFFFFFFFF
#define MAX_TID_SIZE 0x7FFFFFFFFFFFFFFF // also known as MAXTAG in HLD
#define TIDLENGTH (32)
#define OBJSTRSIZE (64)
#define ERRORTYPE unsigned short
#define REALTID(X) ((X) & RESTOREMAGICBIT)
#define MAGICTID(X) ((X) | UNLINKMAGICBIT)

/*
 * MDHIM Range Servers will receive batch processing requests from IOD.
 * RS will try to malloc memory to store required records. Because RS don't
 * know exact how much memory will be consumed, a set of buffers with increasing
 * buffer size will be used until it's large enough.
 * The buffer will be increased by times of MDHIM_BUF_INC_TIMES. The bigger
 * MDHIM_BUF_INC_TIMES is, the smaller number of memory allocation is needed for
 * a given record list size.
 * */

#define MDHIM_INIT_BUF_SIZE (8192)
#define MDHIM_BUF_INC_TIMES (4)
#define OBJ_STR_SIZE (64)

typedef uint64_t        mdhim_trans_id_t;
...
typedef struct MDHIMFD_s {
	...
	pblIsamFile_t *index_isamfd; /* index table fd */
	...
	PblList *tidCache; /* maintains key number for each TID */
	char fd_addr_str[64]; /* TODO workaround for PBL Map. Using this to store
				MDHIMFD_t address. This will be replaced by IOD
				general layer's map functionality in future */
} MDHIMFD_t;
...
typedef enum {
    MDHIM_LOC_BB, /* Burst Buffer */
    MDHIM_LOC_CENTRAL, /* Central storage, DAOS for example */
} MDHIM_CREATE_LOC_t;

typedef struct MDHIM_CREATE_INFO_s {
    MDHIM_CREATE_LOC_t loc;
    int max_key_per_range;  /* 0 means disabled */
    int range_server_num;   /* number of range servers */
    int *range_server_list; /* range server list */
} MDHIM_CREATE_INFO_t;

typedef struct MDHIM_TID_INFO_s {
    uint64_t tid;       /* IOD transaction tid */
    int      count;     /* key number for each tid */
} MDHIM_TID_INFO_t;

extern int mdhimCreate( const char *containerName, char *objId,
			 MDHIM_CREATE_INFO_t *info );
extern int mdhimFinalize();
#if 0
extern int mdhimFind(MDHIMFD_t * fd, int key_indx, int type, void *ikey,
		     int *rec_number, void *okey, int *okey_len);
extern int mdhimFlush(MDHIMFD_t * fd);
extern int mdhimGet(MDHIMFD_t * fd, int key_indx, int ftype, int *record_num,
		    void *okey, int *okey_len);
extern int mdhimDelete(MDHIMFD_t * fd, int keyIndx, void *ikey, int *record_num,
		       void *okey, int *okey_len);
#endif
extern int mdhimInit(MPI_Comm inComm);
extern int mdhimInsert(const char *objID, uint64_t tid,
			struct keyDataList *key_data_list,
		       ...
		       char *rs_list);
extern int mdhimOpen(char *containerName, char *objectName, int mode,
		     int numKeys, int *keyType, int *keyMaxLen, int *keyMaxPad,
		     int maxDataSize, int maxRecsPerRange, int rangeSvrSize,
		     char *rs_list);
extern int mdhimReaddata(char *objIDStr, const char *pkey, char *data,
			int *datalen, int keyType, int max_recs_per_range,
			char *rsl);
extern int mdhimReaddataAt(char *objIDStr, uint64_t tid, off_t offset,
			size_t num, char *data, int *datalen, int *numRecs,
			int keyOnly, char *rsl);
extern int mdhimClose(char *objIDStr, char *rsl);
extern int mdhimUnlink(const char *objID, mdhim_trans_id_t tid,
		struct keyDataList *key_data_list,
	        int num_key_data, LISTRC * ierrors, int rangeSvr_size,
		int nkeys, int pkey_type, int max_recs_per_range,
		char *rs_list);
extern int mdhimGetNum(char *objIDStr, uint64_t tid, char *rsl);
extern int spawn_mdhim_server();
